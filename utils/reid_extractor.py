...
from trackers import LITE, StrongSORT, DeepSORT, OSNet
...
class AppearanceExtractor:
    ...
    def load_model(self):
        if self.tracker == 'LITE':
            ...
            return LITE(model=model, device=self.device, appearance_feature_layer=self.appearance_feature_layer)
    ...
    def extract_features(self):
        ...
        for frame_index in tqdm(frame_indices, desc="Extracting features"):
            ...
            bbox = gt[(gt[0] == frame_index) & (gt[7] == 1)][[2, 3, 4, 5]].values
            ...
            frame_features = pd.DataFrame({'id': track_ids, 'features': [appearance.flatten() for appearance in appearances]})
